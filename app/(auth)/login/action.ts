"use server";
import { MailboxForUser, PasskeyCredentials, ResetPasswordToken, User, db } from "@/db";
...
export default async function signIn(
    ...
    if (typeof verified === "string") {
        ...
        await db.update(User).set({ password: verified }).where(eq(User.id, user.id)).execute();
    }
...
async function handleUserRedirection(
    user: { id: string; onboardingStatus: { initial?: boolean } | null },
    callback?: string | null,
) {
    await addUserTokenToCookie(user);
