...
async function pwdHash(password: string, salt: string, alg = "SHA-512") {
    ...
    try {
        const derivedKey = await crypto.subtle.importKey("raw", encodedPassword, { name: "PBKDF2" }, false, [
            "deriveBits",
        ]);
...
export const verifyPassword = async (proposedPassword: string, passwordHash: string) => {
