...
export default function Loading() {
    return (
        <div className="flex flex-col gap-2 p-5 w-full min-w-0">
            <div className="flex flex-row w-full min-w-0 pb-3 border-b-2 -mt-1 gap-6 animate-pulse">
                <input type="checkbox" disabled id="select" className="mr-2 self-start mt-1 size-4" />
                ...
                <div className="ms-auto me-2 h-5">
                    <span className="inline-block bg-muted-foreground/25 rounded-full p-2 size-5" />
                </div>
            </div>
            ...
            {Array.from({ length: 10 }).map((_, i) => (
                <div key={i} className="rounded shadow-sm h-16 pl-5 pr-5 py-2 w-full flex gap-4 animate-pulse bg-card">
                    ...
                    <span className="self-center w-full hidden sm:flex bg-muted-foreground/25 h-7 shrink-[2] rounded-lg" />
                    <span className="self-center truncate bg-muted-foreground/25 mx-auto shrink-0 rounded-full size-5 -me-2 hidden sm:inline-block" />
                    <span className="float-right self-center text-sm shrink-0 w-16 bg-muted-foreground/25 h-4 rounded text-right" />
