...
export default {
   ...
   * Go to worker > your email worker > settings > variables > add new variable `auth` with the alue provided in the dashboadrd.
   * @param {any} ctx
   */
  async email(message, env, ctx) {
    ...
    const req = await fetch(
      `https://emailthing.xyz/api/recieve-email?zone=${zone}`,
      {
        method: "POST",
        headers: {
