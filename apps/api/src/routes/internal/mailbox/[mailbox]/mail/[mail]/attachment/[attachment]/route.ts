import { db, Email, EmailAttachments, Mailbox, MailboxForUser } from "@/db";
...
export async function GET(request: Bun.BunRequest) {
    ...
    const { mailbox: mailboxId, mail: mailId, attachment: attachmentId } = request.params;
    ...
    const [email] = await db.select({ id: Email.id })
        .from(Email)
    ...
    const [attachment] = await db.select({ id: EmailAttachments.id, filename: EmailAttachments.filename })
