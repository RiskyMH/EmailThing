...
export function EmailthingText({ className, text = "EmailThing" }: { className?: string; text?: string }) {
    return (
        <h2
            className={cn(
                "inline-block whitespace-nowrap from-[#FF9797] to-[#6D6AFF] font-bold text-lg group-hover:bg-gradient-to-br group-hover:bg-clip-text group-hover:text-transparent group-hover:transition-all group-hover:duration-200",
                className,
            )}
        >
            {text}
...
export function EmailThing() {
    return (
        <>
            <Logo className="flex size-7 shrink-0 [.kawaii_&]:hidden" />
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/logo.svg" loading="lazy" className="hidden h-12 self-baseline" alt="EmailThing logo" />
            <EmailthingText className="flex [.kawaii_&]:hidden" />
            <ContextMenu>
                <ContextMenuTrigger className="hidden self-baseline [.kawaii_&]:flex">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                ...
                <ContextMenuContent>
                    <ContextMenuItem asChild className="flex gap-2">
                        <a href="https://github.com/alfonsusac" target="_blank" rel="noreferrer">
