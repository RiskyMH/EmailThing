...
export async function getMailboxName(mailboxId: string) {
  const mailboxAliases = await db.mailboxAliases
    .where("[mailboxId+default]")
    .equals([mailboxId, 1])
    .first();
...
export async function getMailboxCustomDomains(mailboxId: string) {
  const customDomains = await db.mailboxCustomDomains
    .where("mailboxId")
    .equals(mailboxId)
    .toArray();
...
export async function getMailboxUsers(mailboxId: string) {
  const users = await db.mailboxForUser.where("mailboxId").equals(mailboxId).sortBy("joinedAt");
  return users;
}
...
export async function getCurrentUserMailbox(mailboxId: string) {
  const currentUser = await getMe();
  if (!currentUser) return null;
  const mailboxForUser = await db.mailboxForUser
    .where("userId")
    .equals(currentUser.id)
    .and((mailboxForUser) => mailboxForUser.mailboxId === mailboxId)
    .first();
