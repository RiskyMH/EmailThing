    ...
    return (
        <>
            ...
            {Array.from({ length: 25 }).map((_, i) => (
                <div
                    key={i}
                    className="//bg-card flex h-10 w-full animate-pulse gap-3 rounded-md px-2 sm:px-4 py-1.5"
                >
                    <span
...
export function EmailListCategoryLoadingSkeleton() {
    return (
        <>
            ...
            <div className="inline-flex h-6 w-auto max-w-fit items-center justify-center gap-1 border-transparent border-b-3 px-1 font-bold animate-pulse">
                <span className="h-5 w-36 self-center rounded bg-muted-foreground/25" />
            </div>

            <div className="ms-auto me-2 flex h-6 shrink-0 items-center justify-center animate-pulse">
