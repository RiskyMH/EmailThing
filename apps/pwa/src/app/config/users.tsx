...
import { PlusIcon, MoreHorizontalIcon, UserRoundXIcon, Loader2 } from "lucide-react";
import { useTransition, type FormEvent } from "react";
import { toast } from "sonner";
import { DeleteButton } from "./components.client";
import { useParams } from "react-router-dom";
import { getMailbox, getMailboxUsers, getCurrentUserMailbox } from "@/utils/data/queries/mailbox";
...
export default function Users() {
    ...
    const data = useLiveQuery(
        () => Promise.all([getMailbox(mailboxId!), getMailboxUsers(mailboxId!), getCurrentUserMailbox(mailboxId!)]),
        [mailboxId]
    );
                        ...
                        {users?.length ? (
                            ...
                            <TableRow>
                                <TableCell className={`h-24 text-center ${!data ? "fade-in" : ""}`} colSpan={4}>
                                    {data ? "No users yet????" : <Loader2 className="size-8 animate-spin text-muted-foreground mx-auto" />}
                                </TableCell>
                            </TableRow>
                        )}
                    </TableBody>
