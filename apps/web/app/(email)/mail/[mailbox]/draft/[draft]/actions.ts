...
import { DraftEmail, Email, EmailRecipient, EmailSender, Mailbox, MailboxAlias, db } from "@/db";
...
export async function sendEmailAction(mailboxId: string, draftId: string, data: FormData) {
    ...
    if (!mail)
        return {
            error: "Draft doesn't exist, make sure you haven't sent it already",
        };
    ...
    if (!alias)
        return {
            error: "Can't find your alias. Make sure you have access to it.",
        };

    const text = new Turndown().turndown(JSDOM.fragment(html)).replaceAll(/\[(https?:\/\/[^\]]+)\]\(\1\)/g, "$1");
